...
function PersonNode({ data }: { data: { id: string; label: string; isFocus: boolean; gender?: string | null } }) {
  // Gender-based colors
  const genderStyle = 
    data.gender === 'M' ? 'border-blue-500 bg-blue-100 hover:bg-blue-200' :
    data.gender === 'F' ? 'border-pink-500 bg-pink-100 hover:bg-pink-200' :
...
export default function Tree() {
  ...
  const { nodes, rfEdges } = useMemo(() => {
    if (people.length === 0) return { nodes: [], rfEdges: [] };
    ...
    while (queue.length > 0) {
      ...
      // Add parents (going up in generations = negative levels)
      ...
      // Add children (going down = positive levels)
    ...
    // Sort levels for consistent top-to-bottom layout
    ...
    // Create parent-child edges (strong lines showing descent)
    const parentChildEdges = edges
    ...
    // Create marriage edges (dashed lines connecting spouses)
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="card h-[calc(100vh-200px)] overflow-hidden">
        ...
        {!err && mounted && nodes.length === 0 && (
          <div className="p-8 text-center text-gray-600">
            <div className="text-lg mb-2">No family tree data yet</div>
            <div className="text-sm">Add people and relationships to see your tree</div>
          </div>
        )}
      </div>
      ...
      <div className="flex gap-6 text-xs text-gray-600 flex-wrap">
        ...
        <div className="flex items-center gap-2">
          <div className="w-8 h-0.5 bg-pink-500" style={{ backgroundImage: 'repeating-linear-gradient(90deg, #ec4899 0, #ec4899 5px, transparent 5px, transparent 10px)' }}></div>
          <span>Marriage</span>
        </div>
      </div>
    </div>
